...
We used generative AI (Claude) for the following components: drafting and editing the written reports and documentation, assistance with SQL debugging and integration ordering, and verification of our functional-dependency and normalization results using an attribute-closure script (`sql/verify_fds.py`) over all attribute subsets.

The design decisions are ours. The ERD, the choice to model `Review` as a weak entity identified through `EnrolledIn` rather than as an ordinary entity, the disjoint total specialization of `Resource`, the schema, the constraints and the trigger design were decided by us and each of us can explain and defend them. We used the closure script to check conclusions we had already reached by hand rather than to reach them, which is why the errors it surfaced, such as `MeetupWide` failing 3NF on a single non-prime attribute, were corrections to our written argument rather than substitutes for it. AI did not replace our development of the course competencies.
